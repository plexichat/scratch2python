...
def _generate_engine(output_dir, opts=None):
    ...
    lines = [
        ...
        "            while frame:",
        "                local_sp = frame.f_locals.get('sp')",
        "                if local_sp and hasattr(local_sp, 'name'):",
        "                    name = local_sp.name",
        "                    break",
        "                frame = frame.f_back",
        ...
        "                          self._hat_tasks.get(id(g)) != hat_id]",
        '            gen = h["body_gen"](sp)',
        "            if gen is None or not hasattr(gen, '__next__'):",
        "                continue",
        "            self._hat_tasks[id(gen)] = hat_id",
        "            try:",
        ...
        "                    import traceback",
        "                    traceback.print_exc()",
        "        # Clean up _hat_tasks for tasks that were cancelled by the restart",
        "        _live = {id(g) for g in self._tasks}",
        "        for tid in list(self._hat_tasks.keys()):",
        "            if tid not in _live:",
        "                self._hat_tasks.pop(tid, None)",
        "",
        ...
        "        if not targets:",
        "            return",
        "        # Track receiver generators via _task_sprite (they were just spawned by _dispatch_broadcast)",
        "        _wait_ids = set()",
        "        for sp, h in targets:",
        "            hat_id = id(h)",
        "            for g in self._tasks:",
        "                if self._task_sprite.get(id(g)) == sp.name and self._hat_tasks.get(id(g)) == hat_id:",
        "                    _wait_ids.add(id(g))",
        ...
        "        _caller_id = self._current_task_id",
        "        self._tasks = [",
        ...
        "        for tid in list(self._hat_tasks.keys()):",
        "            if tid not in self._task_sprite:",
        "                self._hat_tasks.pop(tid, None)",
        "",
        ...
        "                    self._task_sprite.pop(tid, None)",
        "            for tid in list(self._hat_tasks.keys()):",
        "                if tid not in {id(g) for g in self._tasks}:",
        "                    self._hat_tasks.pop(tid, None)",
        ...
        "            _mx_px = int((_mx + _disp.stage_w / 2) * _sx)",
        "            _my_px = int((_disp.stage_h / 2 - _my) * _sx)",
        "            _ix = _mx_px - _px",
        "            _iy = _my_px - _py",
        "            if 0 <= _ix < _nw and 0 <= _iy < _nh:",
        "                _s = max(0.0, sp.size) / 100.0",
        "                _ox = int(_ix / _s) if _s > 0 else 0",
        "                _oy = int(_iy / _s) if _s > 0 else 0",
        "                if 0 <= _ox < _w and 0 <= _oy < _h:",
        "                    return bool(_arr[_oy, _ox, 3] > 0)",
        "            return False",
        "        except Exception:",
        "            return False",
        "    if obj == '_edge_':",
        ...
        "    try:",
        "        import numpy as _np",
        "        if getattr(_disp, '_last_frame', None) is not None:",
        "            _stage_img = _np.array(_disp._last_frame, copy=True)",
        "        else:",
        "            _stage_img = _np.asarray(_disp.stage)",
        "        if _stage_img.ndim != 3 or _stage_img.shape[2] < 3:",
        "            return False",
        "        if _stage_img.shape[2] >= 4:",
        ...
        "            if _pen_arr.shape[:2] == _stage_img.shape[:2] and _pen_arr.shape[2] >= 4:",
        ...
        "    if md5:",
        "        _disp.stamp(md5, x, y, size, getattr(sp, 'name', None) if sp is not None else None, sp=sp)",
        "    elif DEBUG:",
        ...
        "        if key == 'shade':",
        "            sp.pen_params['brightness'] = max(0.0, min(100.0, _num(val)))",
        "            _hsbt_apply(sp)",
        "            return",
        ...
        "        key = _str(param).lower()",
        "        if key == 'shade':",
        "            sp.pen_params['brightness'] = max(0.0, min(100.0, sp.pen_params['brightness'] + _num(val)))",
        "            _hsbt_apply(sp)",
        "            return",
        "        if key in sp.pen_params:",
        "            if key == 'color':",
...
def _generate_display(output_dir, opts=None):
    ...
    lines = [
        ...
        "        # Use the costume rotation center, or image center if not defined",
        "        _rc = sp._costume_rotation_center() if sp is not None else None",
        "        if _rc is not None:",
        ...
        "        if sp is not None:",
        "            direction = getattr(sp, 'direction', 90)",
        "            rotation_style = getattr(sp, 'rotation_style', 'all around')",
        "            effects = getattr(sp, '_effects', {})",
        "            if rotation_style == 'all around':",
        "                _angle = 90.0 - direction",
        "                _radians = math.radians(_angle)",
        "                _cos = abs(math.cos(_radians))",
        "                _sin = abs(math.sin(_radians))",
        "                _mdx = max(_rcx, nw - _rcx)",
        "                _mdy = max(_rcy, nh - _rcy)",
        "                _rnw = int(_mdx * _cos + _mdy * _sin) * 2 + 1",
        "                _rnh = int(_mdx * _sin + _mdy * _cos) * 2 + 1",
        "                _canvas = Image.new('RGBA', (_rnw, _rnh), (0, 0, 0, 0))",
        "                _cx = (_rnw // 2 - int(_rcx))",
        "                _cy = (_rnh // 2 - int(_rcy))",
        ...
        "                py = int((self.stage_h / 2 - y) * sy - _rcy)",
        "            else:",
        "                px = int((x + self.stage_w / 2) * sx - _rcx)",
        "                py = int((self.stage_h / 2 - y) * sy - _rcy)",
        "            if effects:",
        "                img = self._apply_effects(img, effects)",
        "        else:",
        "            px = int((x + self.stage_w / 2) * sx - nw / 2)",
        "            py = int((self.stage_h / 2 - y) * sy - nh / 2)",
        ...
        "        # Round end caps (Scratch lineCap='round')",
        "        if w > 1:",
        "            _r = w / 2.0",
        "            draw.ellipse([px1 - _r, py1 - _r, px1 + _r, py1 + _r], fill=color)",
        "            draw.ellipse([px2 - _r, py2 - _r, px2 + _r, py2 + _r], fill=color)",
        "        self._pen_layer_version += 1",
        "",
        ...
        "        ImageDraw.Draw(self.pen_layer).ellipse((px - r, py - r, px + r, py + r), fill=color); self._pen_layer_version += 1",
        "",
        "    @staticmethod",
